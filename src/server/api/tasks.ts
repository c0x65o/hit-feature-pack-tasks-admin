...
export async function GET(request: NextRequest) {
  try {
    ...
    const scheduleRows =
      pageNames.length > 0
        ? await db
            ...
            .where(inArray(hitTaskSchedules.taskName as any, pageNames as any) as any)
            .limit(1000)
        : [];
    const scheduleByName = new Map<string, boolean>();
    ...
    const lastRunRows =
      pageNames.length > 0
        ? await db
            ...
            .where(inArray(hitTaskExecutions.taskName as any, pageNames as any) as any)
            .groupBy(hitTaskExecutions.taskName)
        : [];
    const lastRunByName = new Map<string, string>();
    for (const r of lastRunRows as any[]) {
      const name = String(r.taskName);
      const v = String(r.lastCompletedAt || r.lastEnqueuedAt || '').trim();
      if (v) lastRunByName.set(name, v);
    }

    // Recent execution summary for this page (helps show status without extra calls)
    const latestExecRows =
      pageNames.length > 0
        ? await db
            .select({
    ...
    for (const r of latestExecRows as any[]) {
      const name = String(r.taskName);
