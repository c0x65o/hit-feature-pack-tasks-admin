...
function resolveSortColumn(sortBy: string) {
  const k = sortBy;
  if (k === 'task_name' || k === 'taskName') return hitTaskExecutions.taskName;
  if (k === 'status') return hitTaskExecutions.status;
  if (k === 'enqueued_at' || k === 'enqueuedAt') return hitTaskExecutions.enqueuedAt;
  if (k === 'started_at' || k === 'startedAt') return hitTaskExecutions.startedAt;
  if (k === 'completed_at' || k === 'completedAt') return hitTaskExecutions.completedAt;
  if (k === 'duration_ms' || k === 'durationMs') return hitTaskExecutions.durationMs;
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (taskName) conditions.push(eq(hitTaskExecutions.taskName, taskName));
    if (status) conditions.push(eq(hitTaskExecutions.status, status));
    if (search) conditions.push(ilike(hitTaskExecutions.taskName, `%${search}%`));
    ...
    const orderBy = sortOrder === 'asc' ? asc(sortCol as any) : desc(sortCol as any);
