/**
 * Stub for @/lib/feature-pack-schemas
 *
 * This is a type-only stub for feature pack compilation.
 * At runtime, the consuming application provides the actual implementation
 * which is auto-generated from feature pack schemas.
 */

// Re-export from the local schema for type checking during build.
export {
  hitTaskSchedules,
  hitTaskExecutions,
  type HitTaskSchedule,
  type InsertHitTaskSchedule,
  type HitTaskExecution,
  type InsertHitTaskExecution,
} from '../schema/job-core';
